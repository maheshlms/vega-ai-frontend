...
interface CriticalAlerts {
    multipleMFAAttempts: boolean;
    accountLockout: boolean;
    privilegeEscalation: boolean;
    unusualLoginLocation: boolean;
}

interface ActivityNotifications {
    newUserProvisioned: boolean;
    userDeprovisioned: boolean;
    groupMembershipChanged: boolean;
    roleAssignmentChanged: boolean;
    userProfileUpdated: boolean;
}

type EmailFrequency = 'Real-time' | 'Hourly Digest' | 'Daily Digest' | 'Weekly Digest';
...
const NotificationSetting: React.FC = () => {
    ...
    const [emailFrequency, setEmailFrequency] = useState<EmailFrequency>('Real-time');
    ...
    const [slackConnected, setSlackConnected] = useState<boolean>(true);
    ...
    const [alertThreshold, setAlertThreshold] = useState<AlertThreshold>({
        failedLogins: "5",
        timeWindow: '10'
    });
    ...
    const [activityNotifications, setActivityNotifications] = useState<ActivityNotifications>({
    ...
    // Modal States
    const [showSlackConfigModal, setShowSlackConfigModal] = useState<boolean>(false);
    const [showTeamsConfigModal, setShowTeamsConfigModal] = useState<boolean>(false);

    // Handle Email Notification Change
    const handleEmailNotificationChange = (notification: keyof EmailNotifications): void => {
    ...
    const handleCriticalAlertChange = (alert: keyof CriticalAlerts): void => {
    ...
    const handleActivityNotificationChange = (notification: keyof ActivityNotifications): void => {
    ...
    const handleSlackChannelChange = (channel: keyof SlackChannels, value: string): void => {
    ...
    const handleDisconnectTeams = (): void => {
    ...
    const handleSendTestAlert = (): void => {
                        ...
                        <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-t-2xl">
                            <div className="flex justify-between items-center">
                                ...
                                <button
                                    ...
                                    className="text-white hover:bg-white hover:bg-opacity-20 hover:text-red-500 rounded-full p-2 transition-all duration-200"
                                >
                                    <FaTimes size={20} />
                                </button>
                            </div>
