...
const Rename = () => {
  ...
  const { userName, updateUserName } = useUser(); // Get userName and update function from context
  const [first, setFirst] = useState("");
  const [second, setSecond] = useState("");

  const handleSave = () => {
    if (first === second && first.trim() !== "") {
      updateUserName(first); // Update the global state
      navigate("/settings");
    }
  };
          ...
          <div className="px-5 py-4">
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Current Name: <span className="font-bold text-indigo-600">{userName}</span>
              </label>
            </div>
