      ...
      <div className="agc-guided-inner">
        <div className="agc-guided-header">
          <div className="agc-guided-header-left">
            ...
            <div>
              <div className="agc-guided-title">Quick Actions</div>
              <div className="agc-guided-subtitle">Click any action to get started with {agentName}</div>
            </div>
          </div>
          <button className="agc-guided-dismiss" onClick={onDismiss} title="Dismiss">
            <IoClose size={14} />
          </button>
        </div>
...
const TourTooltip: React.FC<TourTooltipProps> = ({ step, stepIndex, totalSteps, targetRect, onNext, onSkip }) => {
...
const AgentChat: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTargetSystem = async () => {
      ...
      if (!targetId) { setTargetSystem(null); return; }
      try {
  ...
  const handleMuteToggle = useCallback(() => {
    ...
    setMuted(newMuted);
    if (audioElRef.current) audioElRef.current.volume = newMuted ? 0 : volumeRef.current;
  }, [muted]);
  ...
  const startListeningOnce = useCallback(() => {
    ...
    if (isTypingRef.current) return;  // don't listen while AI is thinking
    try {
  ...
  // Speech recognition setup — now with persistent mode awareness
  useEffect(() => {
    ...
    rec.onerror = (e: SpeechRecognitionErrorEvent) => {
      setIsListening(false);
      // On error in voice mode, retry after a short pause (unless it was aborted intentionally)
      if (voiceModeActiveRef.current && e.error !== 'aborted') {
        setTimeout(() => {
          if (voiceModeActiveRef.current && !isPlayingRef.current && !isTypingRef.current) {
            startListeningOnce();
          }
        }, 1000);
      }
    ...
    rec.onend = () => {
      setIsListening(false);
      // ── CHANGE: if voice mode is on and avatar is not speaking, restart automatically ──
      if (voiceModeActiveRef.current && !isPlayingRef.current && !isTypingRef.current) {
        setTimeout(() => {
          if (voiceModeActiveRef.current && !isPlayingRef.current && !isTypingRef.current) {
            startListeningOnce();
  ...
  }, [startListeningOnce]); // eslint-disable-line

  // ── CHANGE: Watch isPlaying — when avatar stops speaking, resume listening if voice mode on ──
  useEffect(() => {
    if (!isPlaying && voiceModeActive && !isTyping) {
      // Small delay to let audio fully finish before mic opens
      const t = setTimeout(() => {
        if (voiceModeActiveRef.current && !isPlayingRef.current && !isTypingRef.current) {
          startListeningOnce();
        }
    ...
    // If avatar starts speaking, stop the mic so it doesn't pick up avatar audio
  ...
  // ── CHANGE: Watch isTyping — when AI finishes thinking, resume listening if voice mode on ──
  useEffect(() => {
    if (!isTyping && voiceModeActive && !isPlaying) {
      const t = setTimeout(() => {
        if (voiceModeActiveRef.current && !isPlayingRef.current && !isTypingRef.current) {
          startListeningOnce();
        }
  ...
  const handleVoiceModeToggle = useCallback(() => {
    ...
    if (newActive) {
      // Turning on: start listening immediately (if not playing/typing)
      setTimeout(() => {
        if (!isPlayingRef.current && !isTypingRef.current) {
          startListeningOnce();
        }
      }, 100);
    } else {
      // Turning off: stop any active recognition
      setIsListening(false);
      try { recognitionRef.current?.stop(); } catch (_) { }
    }
  }, [startListeningOnce]);
  ...
  const startListening = useCallback(() => {
    ...
    startListeningOnce();
  }, [isListening, isPlaying, handleInterrupt, startListeningOnce]);
  ...
  const handleDownloadFile = async (message: Message): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}${message.file_path}`, { headers: { 'Authorization': `Bearer ${token}` } });
      if (!response.ok) throw new Error(`Download failed: ${response.statusText}`);
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = message.filename || 'download.txt';
      document.body.appendChild(a); a.click();
      window.URL.revokeObjectURL(url); document.body.removeChild(a);
    } catch (error: any) { alert(`Failed to download file: ${error.message}`); }
    ...
    try {
      ...
      const response = await api.fetchWithAuth('/api/v1/chat', {
        ...
        body: JSON.stringify({
          ...
          agent_type: agent?.type || 'license', message: currentInput,
  ...
  }, [inputValue, chatHistory, agent, attachedFiles, isPlaying, isListening, speakMessage, handleInterrupt]);

  useEffect(() => {
    if (autoSendVoice && inputValue.trim()) { handleSend(); setAutoSendVoice(false); }
              ...
              <div className="agc-avatar-top-bar">
                <span className="agc-status-pill-fixed">
                  <span style={{
                    ...
                    animation: isPlaying ? 'agc-pulse-speaking 0.8s ease-in-out infinite' : 'none',
                  }} />
                ...
                <div className="agc-avatar-top-bar-actions">
                  {!isAvatarActive ? (
                    <button id="tour-start-btn" className="agc-overlay-btn agc-overlay-btn-green" onClick={startSession} disabled={isLoadingSession}>
                      {isLoadingSession ? <><div className="agc-spinner" style={{ width: 8, height: 8, borderTopColor: '#6ee7b7' }} /> Starting…</> : <>▶ Start</>}
                    </button>
                  ) : (
                    <>
          ...
          <div className="agc-chat-panel">
            ...
            <div className="agc-input-zone">
              ...
              <div className="agc-input-wrap">
                ...
                {/* ── CHANGE: Voice toggle button replaces the old single-shot mic icon ── */}
                <button
                  id="tour-voice-btn"
                ...
                >
                  ...
                  {isListening ? 'Listening…' : voiceModeActive ? 'Voice On' : 'Voice'}
                </button>

                <div className="agc-divider" />
                <button id="tour-send-btn" className="agc-send-btn" onClick={handleSend} disabled={isTyping || !inputValue.trim()}>
