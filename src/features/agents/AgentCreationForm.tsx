...
try {
  ...
  // Manifest not yet generated — will fall back to HeyGen CDN URLs or initials
}

export const FALLBACK_AVATARS = [
  { id: 'Marianne_ProfessionalLook_public',   name: 'Marianne (Formal)',   color: '#6366f1' },
  { id: 'Katya_ProfessionalLook_public',      name: 'Katya (Formal)',      color: '#10b981' },
  { id: 'Alessandra_ProfessionalLook_public', name: 'Alessandra (Formal)', color: '#f59e0b' },
  { id: 'Tyler_ProfessionalLook_public',      name: 'Tyler (Formal)',      color: '#fca5a5' },
  { id: 'Anna_public_3_20240108',             name: 'Anna (Formal)',       color: '#8b5cf6' },
  { id: 'Eric_public_pro1_20230608',          name: 'Eric (Formal)',       color: '#06b6d4' },
  { id: 'Susan_public_2_20240328',            name: 'Susan (Formal)',      color: '#ec4899' },
  { id: 'Shelly_public_20240408',             name: 'Shelly (Formal)',     color: '#f97316' },
  { id: 'Wayne_20240711_public',              name: 'Wayne (Formal)',      color: '#14b8a6' },
];
...
interface AvatarOption {
  ...
  img: string;
  color: string;
  variant: 'formal' | 'casual' | 'unknown';
}

// ── FIX: Added `type?` so the target system's integration type is captured ──
interface TargetSystem {
  ...
  type?: string;        // ← e.g. "ping_federate", "ping_directory", "ping_one"
  base_url?: string;
  host?: string;
  hostname?: string;
}
...
interface LocationState {
  ...
  integrationType?: string;   // ← passed from the select-target screen
}

// ── FIX: integrationType added to config payload ──
interface AgentCreationPayload {
  ...
  config: {
    ...
    agentTypeId?: string;
    integrationType?: string;   // ← Agents.tsx reads this to resolve category
    selectedAvatarId?: string;
    selectedAvatarImg?: string;
    selectedAvatarName?: string;
  };
...
function friendlyName(id: string, variant: 'formal' | 'casual' | 'unknown'): string {
  const first = extractFirstName(id);
  const cap = first.charAt(0).toUpperCase() + first.slice(1);
  return cap + (variant === 'formal' ? ' (Formal)' : variant === 'casual' ? ' (Casual)' : '');
}
...
function resolveImg(id: string, cdnUrl: string): string {
  if (avatarManifest[id]) return avatarManifest[id];
  if (cdnUrl && !cdnUrl.match(/\.(mp4|webm|mov)(\?|$)/i)) return cdnUrl;
  return '';
}
...
// ─── Circle avatar ─────────────────────────────────────────────────────────────
const AvatarCircleImage: React.FC<{ avatar: AvatarOption; textSize?: string }> = ({
        ...
        <>
          {state === 'loading' && (
            <div className="absolute inset-0 flex items-center justify-center z-10"
              style={{ background: `${avatar.color}44` }}>
          ...
          <img
            ...
            alt={avatar.name}
            loading="lazy"
            className={`absolute inset-0 w-full h-full object-cover object-top z-20 transition-opacity duration-300 ${
...
const AvatarLargePreview: React.FC<{ avatar: AvatarOption | null }> = ({ avatar }) => {
  ...
  return (
    ...
    >
        ...
        <>
          {state === 'loading' && (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3"
              ...
              <span className="text-white/80 text-xs font-medium">Loading photo…</span>
            </div>
          )}
          <img
            src={avatar.img}
            alt={avatar.name}
            loading="lazy"
            className={`absolute inset-0 w-full h-full object-cover object-top z-20 transition-all duration-500 ${
      ...
      <div
        className="absolute top-3 right-3 z-30 text-white text-xs font-black px-3 py-1 rounded-full shadow-lg"
...
const AvatarCarousel: React.FC<AvatarCarouselProps> = ({ avatars, loading, selectedId, onSelect }) => {
  ...
  const update = React.useCallback(() => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      if (!el) return;
      setCanLeft(prev  => { const n = el.scrollLeft > 2;                                   return prev !== n ? n : prev; });
      setCanRight(prev => { const n = el.scrollLeft < el.scrollWidth - el.clientWidth - 2; return prev !== n ? n : prev; });
    });
  }, []);
    ...
    <div>
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-400">{avatars.length} avatars</span>
      ...
      <p className="text-xs text-gray-500 mb-3">Formal dress avatars only</p>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative flex-1 min-w-0">
          <div className="absolute left-0 top-0 bottom-0 w-10 z-10 pointer-events-none transition-opacity duration-200"
            style={{ background: 'linear-gradient(to right, white 20%, transparent)', opacity: canLeft ? 1 : 0 }} />
          <div className="absolute right-0 top-0 bottom-0 w-10 z-10 pointer-events-none transition-opacity duration-200"
            ...
            {avatars.map(avatar => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <AvatarCircleImage avatar={avatar} textSize="text-xl" />
...
const AgentCreationForm: React.FC = () => {
  ...
  // ── FIX: Read integrationType from navigation state (set by select-target screen) ──
  ...
  const [avatarSource,   setAvatarSource]   = React.useState<'api' | 'fallback'>('api');
  ...
  // ── Fetch formal-only avatars, max 20 ─────────────────────────────────────
  React.useEffect(() => {
    const fetchAvatars = async () => {
      ...
      if (!apiKey) {
        const formalFallback = FALLBACK_AVATARS
          .filter(a => classifyVariant(a.id) === 'formal')
          .slice(0, 20)
          .map((a, i) => ({
            ...a,
            img: avatarManifest[a.id] ?? '',
            variant: 'formal' as const,
            color: COLORS[i % COLORS.length],
          }));
        setAvatars(formalFallback);
      ...
      try {
        ...
        if (!rawAvatars.length) throw new Error('Empty list');

        const allMapped: AvatarOption[] = rawAvatars
          .map((a: any, i: number) => {
            const id = a.avatar_id ?? a.id ?? a.pose_id ?? '';
            if (!id) return null;
            const variant = classifyVariant(id);
            if (variant !== 'formal') return null;
            const cdnUrl =
              a.normal_preview ?? a.preview_image_url ?? a.thumbnail_image_url ??
              a.headshot_image_url ?? a.image_url ?? a.thumbnail ?? a.preview ?? '';
            const safeCdn = cdnUrl && !String(cdnUrl).match(/\.(mp4|webm|mov)(\?|$)/i) ? String(cdnUrl) : '';
            return {
              id, variant,
              name:  friendlyName(id, variant),
              img:   resolveImg(id, safeCdn),
              color: COLORS[i % COLORS.length],
            } as AvatarOption;
          })
        ...
        const sorted = [...allMapped].sort((a, b) => {
          const rank = (av: AvatarOption) =>
            av.img.startsWith('/avatars/') ? 2 : av.img.startsWith('http') ? 1 : 0;
          return rank(b) - rank(a);
        });

        setAvatars(sorted.slice(0, 20));
        setAvatarSource('api');
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // ── FIX: Resolve integrationType with priority chain ──
      // Priority 1: target system's own `type` field (direct from API)
      // Priority 2: integrationType passed via navigate() state from select-target screen
      // Priority 3: agentTypeId URL param as last resort
      ...
      console.log('[AgentCreationForm] integrationType resolved to:', resolvedIntegrationType);
      ...
      const payload: AgentCreationPayload = {
        ...
        config: {
          ...
          agentTypeId,
          integrationType:    resolvedIntegrationType,  // ← KEY: Agents.tsx uses this for PingFederate/etc category
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-6 text-left space-y-3">
              {([
                ...
                ...(agentTypeId !== 'connection'
                  ? [['Notification Window', `${formData.notificationWindow} days`]] : []),
            ...
            <div className="flex-1 min-w-0 bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden">
              <div className="flex items-center justify-between px-8 2xl:px-12 py-6 border-b border-gray-100 bg-gradient-to-r from-white/50 to-blue-50/30">
                <div>
                  ...
                  {/* Show what integration type will be used for categorisation */}
              ...
              <form onSubmit={handleSubmit} className="px-8 2xl:px-12 py-6 2xl:py-8 space-y-7">
                ...
                {!loadingAvatars && avatarSource === 'fallback' && (
                  <div className="rounded-lg bg-amber-50 border border-amber-200 text-amber-700 px-4 py-3 text-sm flex items-start gap-2">
                    <span>⚠️</span>
                    <div>
                      <p className="font-medium">HeyGen API key not configured</p>
                      <p className="text-xs mt-0.5">
                        Set <code className="bg-amber-100 px-1 rounded font-mono">VITE_HEYGEN_API_KEY</code> to load live avatars.
                      </p>
                    </div>
                  </div>
                )}
                ...
                {/* 1. Avatar carousel */}
                <AvatarCarousel
                  avatars={avatars} loading={loadingAvatars}
                  selectedId={formData.selectedAvatarId} onSelect={handleAvatarSelect}
                />
                    ...
                    {formData.selectedTargetSystem && (
                      <div className="mt-2 p-3 bg-green-50 rounded-lg border border-green-200 flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          ...
                          {/* Confirm which integrationType will be saved */}
            ...
            <div className="hidden lg:flex flex-col w-72 2xl:w-80 flex-shrink-0 gap-4 sticky top-8">
              ...
              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-100 p-5">
                ...
                <ul className="space-y-2 text-xs text-indigo-600/90 leading-relaxed">
                  {/* <li className="flex gap-2"><span>•</span><span>All avatars are formal for a professional look</span></li> */}
